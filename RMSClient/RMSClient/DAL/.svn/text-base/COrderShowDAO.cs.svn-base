    ...
    class COrderShowDAO : BaseDAO, IOrderShowDAO //Changed
    {
        ...
        public List<COrderShow> OrderListShowByStatus(string sStatus)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sSql = string.Format(SqlQueries.GetQuery(Query.OrderShowByStatus), sStatus);
        ...
        public CResult UpdateDBForMerge(String[] inOrderID, String inMasterOrderID, int[] inTableNumber, int inMasterTableNumber, int inTotalGuestCount)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < inOrderID.Length; i++)
                {
                    ...
                    {
                        //this.OpenConnection();
                        sSql = String.Format(SqlQueries.GetQuery(Query.DeleteDBForMerge), inOrderID[i], inTableNumber[i], inMasterOrderID);
                        //this.ExecuteReader(sSql);
                        this.ExecuteNonQuery(sSql);
                        //this.CloseConnection();
                    }
                ...
                //this.OpenConnection();
                sSql = String.Format(SqlQueries.GetQuery(Query.UpdateDBForMerge), inMasterOrderID, inMasterTableNumber, inTotalGuestCount);
                //this.ExecuteReader(sSql);
                this.ExecuteNonQuery(sSql);
        ...
        private CTransferOrderShow ReaderToTransferOrderShow(IDataReader oReader)
        {
            ...
            if (oReader["table_number"] != null)
                oTransferOrderShow.TableNumber = int.Parse(oReader["table_number"].ToString());
