  ...
  public  class SystemManager
    {
      ...
      public CResult GetSalesRecordsForINV(Int64 p_fromDate, Int64 p_toDate)
      {
          try
          {
              m_oResult = Database.Instance.SaveDefaultTime.CollectSalesReportRecordsForInv(p_fromDate, p_toDate);
              m_oResult.IsSuccess = true;
      ...
      public CResult GetRawMat(Int64 p_fromDate, Int64 p_toDate)
      {
         try
         {
              m_oResult = Database.Instance.SaveDefaultTime.CollectRawMat(p_fromDate, p_toDate);
              m_oResult.IsSuccess = true;
             m_oResult.Message = "Data Read Successfully";
         ...
         {
              System.Console.WriteLine("Exception occuered at GetRawMat() : " + ex.Message);
              m_oResult.IsException = true;
              m_oResult.Action = EERRORNAME.EXCEPTION_OCCURE;

              m_oResult.Message = ex.Message;
              Logger.Write("Exception : " + ex + " in GetRawMat()", LogLevel.Error, "RawMat");
         }
         return m_oResult;
     }
