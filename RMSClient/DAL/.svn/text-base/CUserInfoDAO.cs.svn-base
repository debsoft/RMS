    ...
    public class CUserInfoDAO : BaseDAO, IUserInfoDAO
    {
        ...
        public CResult LoginUser(CUserInfo inCat)
        {
            ...
            try
            {
                ...
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.LoginUser), inCat.UserName);
                ...
                if (oReader != null)
                {
                    ...
                    if (oReader.Read())
                    {
                        ...
                        if ((!oUser.Password.Equals(String.Empty)) && oUser.Password.Equals(inCat.Password) && oUser.Status != 0)
                        {
                            ...
                            oReader.Close();
                            sqlCommand = string.Format(SqlQueries.GetQuery(Query.CheckCurrentUser), inCat.UserName);
                            ...
                            if (oReader != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    sqlCommand = string.Format(SqlQueries.GetQuery(Query.DeleteCurrentUser), inCat.UserID);

                                    this.ExecuteNonQuery(sqlCommand);

                                    sqlCommand = string.Format(SqlQueries.GetQuery(Query.AddCurrentUser), inCat.UserID, inCat.PCID);
        ...
        public CResult LogoutUser(CUserInfo inUser)
        {
            ...
            {
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.DeleteCurrentUser), inUser.UserID);
                this.ExecuteNonQuery(sqlCommand);
        ...
        public CResult GetUserAccess(CUserInfo inCat)
        {
            ...
            {
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.GetUserAccess), inCat.UserID);
                IDataReader oReader = this.ExecuteReader(sqlCommand);
                if (oReader != null)
                {
                    while (oReader.Read())
        ...
        public CResult LoginAdminUser(CUserInfo inCat)
        {
            ...
            try
            {
                ...
                if (oReader != null)
                {
                    if (oReader.Read())
                    {
                        objUserInfo = ReaderToUserInfo(oReader);

                        if ((!objUserInfo.Password.Equals(String.Empty)) && objUserInfo.Password.Equals(inCat.Password) && objUserInfo.Status != 0)
                        {
                            oResult.IsSuccess = true;

                            oResult.Data = objUserInfo;
                        }
                    }
                }
            }
        ...
        public CResult AddUser(CUserInfo inUser)
        {
            ...
            try
            {
                ...
                if (bTempFlag)
                {
                    objResult.Message = " User exists with this user name. Please write another name.";
                }
                ...
                else
                {
                    ...
                    if (inUser.UserAccess.ProcessDeliveryTime == 1)
                    {
                        sqlCommand = String.Format(SqlQueries.GetQuery(Query.GetButtonIDByName), "OrderProcessTime");
                    ...
                    if (inUser.UserAccess.KitchenText == 1)
                    {
                        sqlCommand = String.Format(SqlQueries.GetQuery(Query.GetButtonIDByName), "KitchenText");
            ...
            catch (Exception ex)
            {
                ...
                objResult.Message = "Could not add User. Please try again";
        ...
        public CResult UpdateUser(CUserInfo inUser)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string sqlCommand = string.Format(SqlQueries.GetQuery(Query.UpdateUser), inUser.UserName, inUser.Password, inUser.Type, inUser.Status, inUser.Gender, inUser.UserID);
                    this.ExecuteNonQuery(sqlCommand);

                    sqlCommand = String.Format(SqlQueries.GetQuery(Query.DeleteAllButtonAccess), inUser.UserID);
                    ...
                    {
                        sqlCommand = String.Format(SqlQueries.GetQuery(Query.GetButtonIDByName), "RemoveItems");
        ...
        private CUserAccess ReaderToUserAccess(IDataReader oReader, CUserAccess oUserAccess)
        {
            ...
            if (oReader["name"] != null)
            {
                ...
                else if (sTempStr.Equals("OrderProcessTime"))
                {
                    oUserAccess.ProcessDeliveryTime = 1;
                }
                else if (sTempStr.Equals("KitchenText"))
                {
                    oUserAccess.KitchenText = 1;
                }
