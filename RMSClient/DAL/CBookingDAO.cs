    ...
    public class CBookingDAO : BaseDAO, IBookingDAO
    {
        ...
        public CResult GetBookingInfoAll(DateTime inCurrentDate)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.BookingInfoGetAll), m_oStartTime, m_oEndTime);
        ...
        public CResult GetBookingInfoByID(Int64 inBookingID)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.BookingInfoGetByID), inBookingID);
        ...
        public CResult InsertBookingInfo(CBooking inBookingInfo)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.BookingInfoInsert), inBookingInfo.BookingType, inBookingInfo.BookingTime, inBookingInfo.CustomerID, inBookingInfo.GuestCount, inBookingInfo.Status, inBookingInfo.Comments, inBookingInfo.TableCount);
        ...
        public CResult UpdateBookingInfo(CBooking inBookingInfo)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.BookingInfoUpdate), inBookingInfo.BookingID, inBookingInfo.BookingType, inBookingInfo.BookingTime, inBookingInfo.CustomerID, inBookingInfo.GuestCount, inBookingInfo.Status, inBookingInfo.Comments, inBookingInfo.TableCount);
        ...
        public CResult DeleteBookingInfo(Int64 inBookingID)
        {
            ...
            try
            {
                ...
                this.OpenConnection();
                string sqlCommand = string.Format(SqlQueries.GetQuery(Query.BookingInfoDelete), inBookingID);
        ...
        private CBooking ReaderToBooking(IDataReader oReader)
        {
            ...
            if (oReader["floororapt_number"] != null)
                oBookingInfo.FloorAptNumber = oReader["floororapt_number"].ToString();

            if (oReader["building_name"] != null)
                oBookingInfo.BuildingName = oReader["building_name"].ToString();

            if (oReader["house_number"] != null)
                oBookingInfo.HouseNumber = oReader["house_number"].ToString();

            if (oReader["phone"] != null)
                oBookingInfo.Phone = oReader["phone"].ToString();

            if (oReader["postal_code"] != null)
                oBookingInfo.CustomerPostalCode = oReader["postal_code"].ToString();

            if (oReader["town"] != null)
                oBookingInfo.CustomerTown = oReader["town"].ToString();

            if (oReader["country"] != null)
                oBookingInfo.CustomerCountry = oReader["country"].ToString();

            if (oReader["street_name"] != null)
                oBookingInfo.StreetName = oReader["street_name"].ToString();
